...
Adapted from EvoScientist's stream.state module for AutoIdea's
research pipeline.
"""
...
class SubAgentState:
    ...
    def add_tool_call(self, name: str, args: dict, tool_id: str = ""):
        """Add or merge a tool call, deduplicating by tool_id."""
    ...
    def add_tool_result(self, name: str, content: str, success: bool = True):
        """Add a tool result and match it to the first unmatched tool call."""
...
class StreamState:
    ...
    def __init__(self):
        ...
        # Cached Markdown object for Rich CLI display (avoids O(n^2) re-parsing)
    ...
    def _get_or_create_subagent(
        ...
        """Smart lookup/creation with rename logic.

        Handles the common case where a sub-agent is first seen as the generic
        "sub-agent" placeholder and later receives its real name, as well as
